...
void Dijkstra(const vector<vector<int>>& graph, int start, int target) {
    int V = graph.size();           // Numero di nodi nel grafo
    ...
    while (!minHeap.empty()) {
        ...
        for (int v = 0; v < V; v++) {
            if (graph[u][v] && !visited[v]) {           // Controlla se esiste un arco e se il nodo non è visitato
